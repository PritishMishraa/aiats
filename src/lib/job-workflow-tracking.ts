...
export async function startJobLifecycleStep(
  jobId: string,
  key: string,
  title: string,
  description: string,
  ...
  const [generation] = await getDb()
    .select({ id: jobGenerationRuns.id })
    .from(jobGenerationRuns)
    .where(eq(jobGenerationRuns.jobId, jobId))
    .limit(1);
  if (!generation) return null;
  const [step] = await getDb()
    .insert(workflowSteps)
    .values({
      jobGenerationId: generation.id,
      key,
      title,
      description,
      position,
...
export async function completeJobLifecycleStep(
  tracking: NonNullable<Awaited<ReturnType<typeof startJobLifecycleStep>>>,
  nextStatus: string,
  ...
  await Promise.all([
    getDb()
      ...
      .where(eq(workflowSteps.id, tracking.stepId)),
    getDb()
      .update(jobGenerationRuns)
      .set({
        status: nextStatus,
      ...
      .where(eq(jobGenerationRuns.id, tracking.generationId)),
  ]);
}

export async function failJobLifecycleStep(
  tracking: NonNullable<Awaited<ReturnType<typeof startJobLifecycleStep>>>,
  error: unknown,
) {
