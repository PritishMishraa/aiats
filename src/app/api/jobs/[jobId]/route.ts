...
export async function PATCH(request: Request, { params }: { params: Promise<{ jobId: string }> }) {
  try {
    ...
    const spec = jobSpecSchema.parse({ ...existing.jobSpec, ...body, location: { ...existing.jobSpec.location, ...body.location }, experience: { ...existing.jobSpec.experience, ...body.experience } });
    const [job] = await getDb().update(jobs).set({ title: spec.title, jobSpec: spec, renderedHtml: renderJob(spec), rubric: null, rubricVersion: 0, status: "draft", updatedAt: new Date() }).where(eq(jobs.id, jobId)).returning();
    await getDb().insert(auditEvents).values({ entityType: "job", entityId: jobId, action: "edited", actor, metadata: { rubricInvalidated: Boolean(existing.rubric) } });
